...
export default function MasterRatesModal({ onClose, onSaved }) {
  ...
  const [rates, setRates] = useState(() => getStoredMasterRates());
  const [activeTab, setActiveTab] = useState(0);
  const [saved, setSaved] = useState(false);
  const [customFormKey, setCustomFormKey] = useState(null); // sub.key of the open "+ Add Custom Finish" form, or null
  ...
  const submitCustomFinish = (catKey, subKey) => {
    ...
    setRates((prev) => addCustomTier(prev, catKey, subKey, trimmed, isNaN(rateNum) ? 0 : rateNum));
    setSaved(false);
    cancelCustomForm();
  };

  const deleteCustomFinish = (catKey, subKey, tierId) => {
    setRates((prev) => removeCustomTier(prev, catKey, subKey, tierId));
    setSaved(false);
  };

  const doSave = () => {
    saveMasterRates(rates);
    setSaved(true);
    if (onSaved) onSaved(rates);
  ...
  // ── Change PIN screen (shown before auth, launched from PIN gate) ──
  if (showChangePin) {
    return (
      <ChangePinScreen
